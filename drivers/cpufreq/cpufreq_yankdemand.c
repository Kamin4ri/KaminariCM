...
#define DEF_INPUT_BOOST					(1094400)/* Yank555.lu : new */
#define DEF_IO_IS_BUSY					(1)	/* Yank555.lu : new */
#define DEF_OPTIMAL_FREQUENCY				(1344400)/* Yank555.lu : new */
...
#define DEF_STEP_UP_EARLY_HISPEED			(1305600)
...
// get_cpu_idle_time_jiffy and get_cpu_idle_time have been moved to cpufreq.c.
				      ...
				      const char *buf, size_t count)
{
	...
	for_each_online_cpu(j) {
		...
		dbs_info->prev_cpu_idle = get_cpu_idle_time(j,
						&dbs_info->prev_cpu_wall, DEF_IO_IS_BUSY);
...
/* PATCH : SMART_UP */
	...
	for_each_cpu(j, policy->cpus) {
		...
		cur_idle_time = get_cpu_idle_time(j, &cur_wall_time, DEF_IO_IS_BUSY);
...
static void dbs_refresh_callback(struct work_struct *work)
{
	...
	if (policy->cur < target_freq) {
		...
		this_dbs_info->prev_cpu_idle = get_cpu_idle_time(cpu,
				&this_dbs_info->prev_cpu_wall, DEF_IO_IS_BUSY);
	}
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			j_dbs_info->prev_cpu_idle = get_cpu_idle_time(j,
						&j_dbs_info->prev_cpu_wall, DEF_IO_IS_BUSY);
			if (dbs_tuners_ins.ignore_nice)
				j_dbs_info->prev_cpu_nice =
